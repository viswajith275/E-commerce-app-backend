...
from sqlalchemy import Enum, ForeignKey, String, or_
...
class ImageValidatorSchema(BaseModel):
    ...
    def validate_size(cls, v):
        if v > MAX_IMAGE_SIZE:
            raise ValueError(
                f"File too large. Max size is {MAX_IMAGE_SIZE / 1024 / 1024}MB"
            )
        return v
...
class User(Base):
    ...
    all_bids: Mapped[List["Bid"]] = relationship(
        back_populates="bider", cascade="all, delete-orphan"
    )
    selled_item_transactions: Mapped[List["Transaction"]] = relationship(
        foreign_keys="[Transaction.seller_id]", back_populates="seller"
    )
    buyed_item_transactions: Mapped[List["Transaction"]] = relationship(
        foreign_keys="[Transaction.bider_id]", back_populates="bider"
    )
...
class UserToken(Base):
    ...
    access_key: Mapped[Optional[str]] = mapped_column(
        nullable=True
    )  # change when making access statable object
...
class Transaction(Base):
    ...
    status: Mapped[TransactionStatus] = mapped_column(
        default=TransactionStatus.MEET_PENDING
    )
    ...
    seller: Mapped["User"] = relationship(
        foreign_keys=[seller_id], back_populates="selled_item_transactions"
    )
    bider: Mapped["User"] = relationship(
        foreign_keys=[bider_id], back_populates="buyed_item_transactions"
    )
