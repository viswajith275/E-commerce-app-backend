...
def Fetch_All_Transactions(current_user: UserDep, db: SessionDep):
    ...
    transactions = (
        current_user.selled_item_transactions + current_user.buyed_item_transactions
    )
...
def Create_Transaction(
    current_user: UserDep, db: SessionDep, transaction_data: TransactionCreate
):
    ...
    bid = (
        ...
        .join(Item)
        .filter(
            Item.id == transaction_data.item_id,
            Item.status == ItemStatus.ACTIVE,
            Item.seller_id == current_user.id,
        )
        .filter(Bid.id == transaction_data.bid_id)
        .first()
    )
    ...
    if not bid:
        raise HTTPException(
            ...
            detail="The item not found or already been sold!",
        )
    try:
        ...
        rejected_bids = (
            db.query(Bid).filter(Bid.item_id == bid.item_id, Bid.id != bid.id).all()
        )
        ...
        new_transaction = Transaction(
            item_id=bid.item_id,
            bid_id=bid.id,
            seller_id=bid.item.seller_id,
            bider_id=bid.bider_id,
            final_price=bid.bid_price,
            status=TransactionStatus.MEET_PENDING,
        )
        ...
        seller_rating = Rating(
            ...
            rater_id=new_transaction.seller_id,
            rated_id=new_transaction.bider_id,
        )
        bider_rating = Rating(
            transaction_id=new_transaction.id,
            rater_id=new_transaction.bider_id,
            rated_id=new_transaction.seller_id,
        )
        ...
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"Transction failed!"
        )
