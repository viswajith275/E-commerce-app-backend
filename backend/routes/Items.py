...
from backend.utils import delete_images, save_image, validate_image
...
def Fetch_All_Items(
    ...
    items = (
        ...
        .limit(limit)
        .all()
    )  # have to change the ordering to catagory or something with query variable
    ...
    for item in items:
        primary_photo_path = (
            db.query(ItemImage)
            .filter(ItemImage.is_primary, ItemImage.item_id == item.id)
            .first()
        )
        ...
        result.append(
            {
                ...
                "status": item.status,
                "images": [
                    image.image_path for image in item.images if not image.is_primary
                ],
            }
        )
...
def Fetch_My_Items(
    ...
    for item in items:
        primary_photo_path = (
            db.query(ItemImage)
            .filter(ItemImage.item_id == item.id, ItemImage.is_primary)
            .first()
        )
...
async def Createt_Item(
    ...
    pending_rating = (
        db.query(Rating)
        .filter(
            Rating.rater_id == current_user.id, Rating.status == RatingStatus.PENDING
        )
        .first()
    )
    ...
    if pending_rating:
        raise HTTPException(
            ...
            detail="Complete pending ratings to create a Item!",
        )

    if price < 0:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST,
            detail="The price should be greater than or equal to 0!",
        )
    ...
    for file in images:
        try:
            ...
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="Please upload a images within the given constraints!",
            )

    for index, file in enumerate(images):
        saved_image_path = await save_image(file)
        new_image = ItemImage(
            item_id=new_item.id, image_path=saved_image_path, is_primary=index == 0
        )
        db.add(new_image)
        db.commit()
...
async def Update_Item(
    ...
    if pending_rating:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Complete pending ratings to create a bid!",
        )

    item = (
        db.query(Item).filter(Item.id == id, Item.seller_id == current_user.id).first()
    )
    ...
    if images is not None:
        prev_img_paths = [
            image.image_path[22::] for image in item.images
        ]  # have to change it cannot give a constant no to strip file path
        ...
        for index, file in enumerate(images):
            saved_image_path = await save_image(file)
            new_image = ItemImage(
                item_id=item.id, image_path=saved_image_path, is_primary=index == 0
            )
            db.add(new_image)
            db.commit()
