...
def Fetch_My_Ratings(current_user: UserDep, db: SessionDep):
    ...
    if not ratings:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No ratings found!"
        )
...
def Fetch_My_Pending_Rating(current_user: UserDep, db: SessionDep):
    ...
    if not ratings:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND, detail="No pending ratings found!"
        )
...
def Update_Rating(current_user: UserDep, db: SessionDep, rating_data: RatingCreate):
    ...
    rating = (
        db.query(Rating)
        .filter(
            Rating.id == rating_data.rating_id,
    ...
    rating.rated_user.rating = round(
        rating.rated_user.total_rating / rating.rated_user.rating_count, 2
    )
